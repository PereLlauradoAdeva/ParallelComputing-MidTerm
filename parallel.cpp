...
// Parallel dilation using OpenMP with DYNAMIC scheduling
...
// Parallel erosion using OpenMP with DYNAMIC scheduling
...
// Opening = erosion followed by dilation (both parallel dynamic)
void Opening_Parallel_Dynamic(const std::vector<unsigned char>& input,
    ...
    Erode_Parallel_Dynamic(input, temp, width, height, kernel_size);
    Dilate_Parallel_Dynamic(temp, output, width, height, kernel_size);
}
